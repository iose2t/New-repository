...
function remove_empty_items(array){ //removes empty items from an array
...
function generate_words() {  
  ...
  var newItemsForStats = TEST_TAB.getRange(1,1,TEST_TAB.getLastRow(),2).getValues(); //captures new words added by the user and sends them to the stats
  var stats = STATS_TAB.getRange(2, 1,STATS_TAB.getLastRow()-1,5).getValues();
      
  //cleans the data
  remove_empty_items(correctWords);
  remove_empty_items(wrongWords);
  remove_empty_items(archive);
  remove_empty_items(inItems);
  remove_empty_items(newItemsForStats);
  remove_empty_items(stats);
  ...
  //if correct answers, increases the number of succeeded in the stats, iterates each in the archive and in case finds it, it removes (splice) it 
  ...
  if (archive.length > 0){    
    shuffle_items(archive);//...shuffles to change the first position
